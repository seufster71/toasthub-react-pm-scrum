...
export function init({parent,parentType}) {
  return function(dispatch) {
    ...
    requestParams.service = "PM_SCRUM_SVC";
    requestParams.prefTextKeys = new Array("PM_SCRUM_PAGE");
    requestParams.prefLabelKeys = new Array("PM_SCRUM_PAGE");
    if (parent != null) {
		...
		dispatch({type:"PM_SCRUM_ADD_PARENT", parent, parentType});
	} else {
		dispatch({type:"PM_SCRUM_CLEAR_PARENT"});
	}
    let params = {};
    ...
    return callService(params).then( (responseJson) => {
    	if (responseJson != null && responseJson.protocalError == null){
    		dispatch({ type: "LOAD_INIT_PM_SCRUM", responseJson });
		} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		dispatch({type:"PM_SCRUM_PREF_CHANGE", userPrefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/member/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "LOAD_LIST_PM_SCRUM", responseJson, paginationSegment });
				if (info != null) {
...
export function listLimit({state,listLimit}) {
	return function(dispatch) {
		 dispatch({ type:"PM_SCRUM_LISTLIMIT",listLimit});
...
export function search({state,searchCriteria}) {
	return function(dispatch) {
		 dispatch({ type:"PM_SCRUM_SEARCH",searchCriteria});
...
export function modifyItem({id,appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "PM_SCRUM_SVC";
	    requestParams.prefFormKeys = new Array("PM_SCRUM_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
	    ...
	    return callService(params).then( (responseJson) => {
	    	if (responseJson != null && responseJson.protocalError == null){
	    		dispatch({ type: 'PM_SCRUM_ITEM',responseJson,appPrefs});
	    	} else {
	    		actionUtils.checkConnectivity(responseJson,dispatch);
	    	}
	    }).catch(error => {
...
export function inputChange(field,value) {
	 return function(dispatch) {
		 ...
		 params.field = field;
		 params.value = value;
		 dispatch({ type:"PM_SCRUM_INPUT_CHANGE",params});
	 };
}

export function orderBy({state,orderCriteria}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_SCRUM_ORDERBY",orderCriteria});
...
export function clearItem() {
	return function(dispatch) {
		dispatch({ type:"PM_SCRUM_CLEAR_ITEM"});
	};
}
...
export function clearField(field) {
	return function(dispatch) {
		let params = {};
		 params.field = field;
		dispatch({ type:"PM_SCRUM_CLEAR_FIELD",params});
	};
}

export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"PM_SCRUM_SET_ERRORS",errors});
	 };
}

export function openDeleteModal({item}) {
	 return function(dispatch) {
		 dispatch({type:"PM_SCRUM_OPEN_DELETE_MODAL",item});
	 };
}

export function closeDeleteModal() {
	 return function(dispatch) {
		 dispatch({type:"PM_SCRUM_CLOSE_DELETE_MODAL"});
	 };
}